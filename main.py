...
def user_post_page(
    ...
    result = db.execute(select(models.Post).where(models.Post.user_id == models.User.id))
    posts = result.scalars().all()
    return templates.TemplateResponse(
        request,
        "user_post.html",
...
@app.get("/api/users/{user_id}", response_model=PostResponse)
def get_post(user_id: int, db: Annotated[Session, Depends(get_db)]):
    result = db.execute(select(models.User).where(models.User.id == user_id))
    existing_users = result.scalar().first()
    if existing_users:
